...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        obs_normalized =  ptu.from_numpy(normalize(obs_unnormalized, obs_mean, obs_std))   # (Q1)
        acs_normalized =  ptu.from_numpy(normalize(acs_unnormalized, acs_mean, acs_std))  # (Q1)
        ...
        delta_pred_normalized =  self.delta_network(concatenated_input)    # (Q1)

        next_obs_pred =  obs_unnormalized + unnormalize(ptu.to_numpy(delta_pred_normalized), delta_mean, delta_std) # (Q1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        target_normalized = normalize(next_observations-observations, data_statistics['delta_mean'], data_statistics['delta_std'])
        target_normalized = ptu.from_numpy(target_normalized)

        # TODO(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
        _, delta_pred_normalized = self.forward(observations, actions, **data_statistics)
        loss =  self.loss(delta_pred_normalized, target_normalized)
