...
from cs285.infrastructure.replay_buffer import ReplayBuffer, normalize, unnormalize
...
class PGAgent(BaseAgent):
    ...
    def estimate_advantage(self, obs: np.ndarray, rews_list: np.ndarray, q_values: np.ndarray, terminals: np.ndarray):
        ...
        if self.nn_baseline:
            ...
            #values = values_unnormalized*values_unnormalized.std() + values_unnormalized.mean()
            ...
            if self.gae_lambda is not None:
                ...
                for i in reversed(range(batch_size)):
                  ...
                  if terminals[i] == 1:
                    advantages[i] = rews[i] + self.gamma*values[i+1] - values[i] # Eq. 15

                  else: 
                    delta_t = rews[i] + self.gamma*values[i+1] - values[i] # Eq. 15 
                    advantages[i] = delta_t + self.gamma*self.gae_lambda*advantages[i+1] # Eq. 21
    ...
    def _discounted_return(self, rewards):
        ...
        T = len(rewards)
        discounted_return = sum([self.gamma**t_prime * rewards[t_prime] for t_prime in range(T)])
    ...
    def _discounted_cumsum(self, rewards):
        ...
        for t in range(T):
          discounted_cumsum = sum([self.gamma**(t_prime-t) * rewards[t_prime] for t_prime in range(t,T)])
