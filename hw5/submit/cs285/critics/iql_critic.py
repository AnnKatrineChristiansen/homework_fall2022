...
class IQLCritic(BaseCritic):
    ...
    def expectile_loss(self, diff):
        ...
        """
        indicator = torch.where(diff<=0, 1, 0)
        L2 =  torch.pow(diff, 2) * torch.abs(self.iql_expectile-indicator)
    ...
    def update_v(self, ob_no, ac_na):
        ...
        ### YOUR CODE HERE ###
        v = self.v_net(ob_no)
        #q = self.q_net_target(ob_no, ac_na).detach()
        q_a_t = self.q_net_target(ob_no)
        q = torch.gather(q_a_t, 1, ac_na.unsqueeze(1))
    ...
    def update_q(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        #q = self.q_net(ob_no, ac_na)
        q_a_t = self.q_net(ob_no)
        q = torch.gather(q_a_t, 1, ac_na.unsqueeze(1))
        targets = reward_n.unsqueeze(1) + self.gamma * v
